def build_night_plan(
    ...
    if recommended_target:
        target_sequence.append(
            {
                "object": recommended_target["object"],
                "start": recommended_target["recommended_start"],
                "end": recommended_target["recommended_end"],
                "reason": recommended_target["next_action"],
            }
        )

    notes = []

    clouds = weather.get("cloud_cover_percent")

    if clouds is not None and clouds > 50:
        notes.append(
            "Cloud cover may significantly reduce imaging quality."
        )

    if recommended_target:
        if not recommended_target["observable"]:
            notes.append(
                f'{recommended_target["object"]} is currently below the horizon.'
            )

        moon_warning = recommended_target.get("moon_warning")

        if moon_warning and not moon_warning.startswith("None"):
            if not moon_warning.startswith("Minimal"):
                notes.append(moon_warning)
